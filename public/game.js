...
// ── Bid panel ─────────────────────────────────
function openBidPanel(current,canPass,hand,b2Active){
  const isB2Active = b2Active || current==='B2';
  const displayBid = isB2Active?'B2':current;
  ...
  // Use latest hand (fallback to myHand so discard updates are shown)
  const displayHand=(hand&&hand.length>0)?hand:myHand;
  const hp=$('hp-cards');hp.innerHTML='';
  displayHand.forEach(c=>hp.appendChild(mkCard(c)));
  // Show/hide discard button
  setDiscardBtn(canDiscardAll);
  showOv('ov-bid');sfxBid();
  // Stop any running timer first, then restart fresh 30s
  stopBidTimer();
...
function showB1Buttons(show){
  const w=$('b1-wrap');if(w)w.classList.toggle('show',!!show);
}
function onB1Call(){
  if(!b1MyTurn)return;
  b1MyTurn=false;showB1Buttons(false);
  socket.emit('b1Call');
  toast('🎯 Bonus in 1 Hand called!',2500);
}
function onB1Skip(){
  if(!b1MyTurn)return;
  b1MyTurn=false;showB1Buttons(false);
...
socket.on('yourCallingTurn',({currentBid:cb,canPass,hand,afterDiscard,b2Active})=>{
  currentBid=cb;
  if(hand&&hand.length>0) myHand=hand;
  openBidPanel(cb,canPass,myHand,b2Active);
  if(afterDiscard){
    const msg=canDiscardAll?'🔄 Still no face card — discard again or bid':'✅ Face card received — now bid!';
    toast(msg,2500);
  }
});
socket.on('bidEvent',({type,pos,name,bid,isB2})=>{
  if(type==='pass'){
    bidLog.push({name,bid:'nil'});
    toast(`${name} passed`);$('status').textContent=`${name} passed`;
  } else if(type==='bid'){
    const displayBid=isB2?'B2':bid;
    bidLog.push({name,bid:displayBid});
    currentBid=isB2?'B2':bid;
    currentBidder=pos;
    toast(`${name} bid ${displayBid}!`);sfxBid();
    $('status').textContent=`${name} bid ${displayBid}`;
    if(isB2) toast('🔥 B2 called — Bonus in 2 Hands! Others must pass.',3000);
  } else if(type==='cardReturned'){
    toast(`${name}'s power card returned`);
  }
...
// B1 Phase events
socket.on('b1PhaseStarted',({firstPos,firstName})=>{
  $('status').textContent=`${firstName} — Bonus in 1 Hand opportunity!`;
});
socket.on('b1TurnChanged',({pos,name})=>{
  $('status').textContent=`${name} — Bonus1 or Skip?`;
  b1MyTurn=(pos===myPos);showB1Buttons(b1MyTurn);
  if(!b1MyTurn) showB1Buttons(false);
});
socket.on('yourB1Turn',({pos,hand})=>{
  b1MyTurn=true;showB1Buttons(true);
  myHand=hand;renderHand();
});
socket.on('b1Called',({callerPos,callerName,frozenPos,frozenName})=>{
  b1MyTurn=false;showB1Buttons(false);
...
socket.on('b1PhaseEnded',({called,callerPos,frozenPos})=>{
  b1MyTurn=false;showB1Buttons(false);
  if(!called){
    $('status').textContent='B1 phase ended — starting game!';
...
// Playing
socket.on('playingStarted',({currentPlayer:cp,currentPlayerName,trickNumber,b1FrozenPos,bidType,isReconnect})=>{
  setActiveAv(cp);
  $('status').textContent=isReconnect?`Trick ${trickNumber} — ${currentPlayerName}'s turn`:`${currentPlayerName} leads Trick 1`;
  updateTrickNum(trickNumber);
  if(bidType==='b1'&&b1FrozenPos>=0){
    showFrozenBadge(b1FrozenPos);
    if(!isReconnect && b1FrozenPos===myPos) toast('❄ You are frozen this round — watch the game!',4000);
  }
  if(!isReconnect && bidType==='b2') toast('🎯 Bonus in 2 Hands — your team needs 10 tricks!',3500);
  if(!isReconnect && bidType==='b1') toast('🎯 Bonus in 1 Hand — 3-player game! 10 tricks needed!',3500);
  if(isReconnect){
    // Re-render hand and show game screen
    renderHand();
    showScreen('screen-game');
    toast('✅ Game state restored!',2000);
  }
});
...
// Restore tricks tally on reconnect
socket.on('tricksUpdate',({tricksWon})=>{
  updateTricks(tricksWon);
});
